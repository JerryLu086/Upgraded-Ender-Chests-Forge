...
public class SpatialEnderChestTile extends CustomChestTileBase {
    public SpatialEnderChestTile(BlockPos pos, BlockState state) {
        super(ModTiles.SPATIAL_ENDER_CHEST.get(), pos, state);
    }

    @Override
    public boolean isOwnContainer(Player player) {
        if (player.containerMenu instanceof ChestMenu menu && menu.getContainer() instanceof SpatialContainer container) {
            return container.activeChest == SpatialEnderChestTile.this;
        } else {
            return false;
        }
    }
